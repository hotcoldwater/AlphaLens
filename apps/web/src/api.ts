...
export async function getStrategyBacktests(strategyId: string): Promise<BacktestRunSummary[]> {
  const response = await request<{ runs: BacktestRunSummary[] }>(`/api/v1/strategies/${encodeURIComponent(strategyId)}/backtests`);
