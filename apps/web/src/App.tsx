...
import { confirmDraft, explainBacktest, getBacktest, getStrategyLibrary, getStrategyVersions, parseStrategy, runDraftBacktest, updateDraft } from "./api";
import type { BacktestExplanation, BacktestResult, Condition, EquityPoint, OHLCVBar, Strategy, StrategyDraft, StrategyLibraryItem, StrategyVersion } from "./types";
...
function linePath(points: EquityPoint[], min: number, span: number) {
  return points.map((point, index) => { const x = points.length === 1 ? 50 : (index / (points.length - 1)) * 100; const y = 88 - ((point.equity - min) / span) * 72; return `${index === 0 ? "M" : "L"}${x.toFixed(2)},${y.toFixed(2)}`; }).join(" ");
}

function Chart({ points, benchmarkPoints = [] }: { points: EquityPoint[]; benchmarkPoints?: EquityPoint[] }) {
  if (!points.length) return <div className="empty-chart">자산곡선 데이터가 없습니다.</div>;
  const values = [...points, ...benchmarkPoints].map((point) => point.equity); const min = Math.min(...values); const max = Math.max(...values); const span = max - min || 1;
  const path = linePath(points, min, span); const benchmarkPath = benchmarkPoints.length ? linePath(benchmarkPoints, min, span) : "";
  const last = points.at(-1)!;
  return <div className="chart-wrap"><div className="chart-legend"><span><i className="strategy-dot" />전략</span>{benchmarkPath && <span><i className="benchmark-dot" />Buy & Hold</span>}</div><svg viewBox="0 0 100 100" preserveAspectRatio="none" role="img" aria-label="전략과 Buy and Hold 자산곡선"><defs><linearGradient id="area" x1="0" x2="0" y1="0" y2="1"><stop offset="0%" stopColor="#d97842" stopOpacity="0.42" /><stop offset="100%" stopColor="#d97842" stopOpacity="0" /></linearGradient></defs><path d={`${path} L100,100 L0,100 Z`} fill="url(#area)" /><path d={benchmarkPath} fill="none" stroke="#41747e" strokeWidth="1.3" strokeDasharray="4 3" vectorEffect="non-scaling-stroke" /><path d={path} fill="none" stroke="#d97842" strokeWidth="1.8" vectorEffect="non-scaling-stroke" /></svg><div className="chart-meta"><span>{points[0].date}</span><strong>{formatWon(last.equity)}</strong><span>{last.date}</span></div></div>;
}

function DrawdownChart({ points }: { points: EquityPoint[] }) {
  if (!points.length) return null;
  let peak = points[0].equity;
  const drawdowns = points.map((point) => { peak = Math.max(peak, point.equity); return { ...point, equity: point.equity / peak - 1 }; });
  const path = drawdowns.map((point, index) => { const x = drawdowns.length === 1 ? 50 : (index / (drawdowns.length - 1)) * 100; const y = 12 + (Math.abs(point.equity) / Math.max(...drawdowns.map((item) => Math.abs(item.equity)), 0.001)) * 76; return `${index === 0 ? "M" : "L"}${x.toFixed(2)},${y.toFixed(2)}`; }).join(" ");
  const lowest = Math.min(...drawdowns.map((point) => point.equity));
  return <div className="drawdown-wrap"><svg viewBox="0 0 100 100" preserveAspectRatio="none" role="img" aria-label="전략 낙폭"><path d={`${path} L100,0 L0,0 Z`} fill="#41747e" fillOpacity=".2" /><path d={path} fill="none" stroke="#41747e" strokeWidth="1.6" vectorEffect="non-scaling-stroke" /></svg><div className="chart-meta"><span>최대 낙폭</span><strong>{percent.format(lowest)}</strong><span>0%</span></div></div>;
}
...
function ResultView({ result, restart, openLibrary }: { result: BacktestResult; restart: () => void; openLibrary: () => void }) {
  const returnTone = result.total_return >= 0 ? "warm" : "cool";
  const [explanation, setExplanation] = useState<BacktestExplanation | null>(null); const [explaining, setExplaining] = useState(false); const [explanationError, setExplanationError] = useState("");
  async function loadExplanation() { setExplaining(true); setExplanationError(""); try { setExplanation(await explainBacktest(result.backtest_id)); } catch (caught) { setExplanationError(caught instanceof Error ? caught.message : "AI 설명을 불러오지 못했습니다."); } finally { setExplaining(false); } }
  return <main className="result-shell"><div className="top-actions"><button className="text-button" onClick={restart}>새 전략 만들기</button><button className="text-button" onClick={openLibrary}>전략 보관함</button></div><section className="hero"><div><p className="eyebrow">ALPHALENS / BACKTEST RESULT</p><h1>전략이 남긴<br />시간의 궤적.</h1><p className="hero-copy">확정된 규칙으로 계산된 결과입니다. AI 해석과 수익률 계산은 분리되어 있습니다.</p></div><div className="run-stamp"><span>RUN STATUS</span><strong>{result.status}</strong><small>{result.backtest_id}</small></div></section><section className="metrics-grid"><Metric label="누적 수익률" value={percent.format(result.total_return)} tone={returnTone} /><Metric label="최종 자산" value={formatWon(result.final_equity)} /><Metric label="CAGR" value={percent.format(result.cagr)} /><Metric label="최대 낙폭" value={percent.format(result.max_drawdown)} tone="cool" /></section><section className="content-grid"><article className="panel equity-panel"><div className="panel-head"><div><span className="panel-kicker">EQUITY CURVE</span><h2>자산곡선 비교</h2></div><span>초기 자산 {formatWon(result.initial_cash)}</span></div><Chart points={result.equity_curve} benchmarkPoints={result.benchmark_equity_curve} /></article><article className="panel profile-panel"><span className="panel-kicker">STRATEGY PROFILE</span><h2>성과의 질</h2><dl><div><dt>샤프지수</dt><dd>{result.sharpe_ratio.toFixed(2)}</dd></div><div><dt>연환산 변동성</dt><dd>{percent.format(result.volatility)}</dd></div><div><dt>승률</dt><dd>{percent.format(result.win_rate)}</dd></div><div><dt>평균 보유기간</dt><dd>{result.average_holding_days.toFixed(1)}일</dd></div><div><dt>총 거래비용</dt><dd>{formatWon(result.total_cost)}</dd></div><div><dt>거래 횟수</dt><dd>{result.trade_count}회</dd></div></dl></article></section><section className="comparison-grid"><article className="panel drawdown-panel"><span className="panel-kicker">DRAWDOWN</span><h2>낙폭의 깊이</h2><DrawdownChart points={result.equity_curve} /></article><article className="panel benchmark-panel"><span className="panel-kicker">REFERENCE</span><h2>Buy & Hold 비교</h2><p>동일 OHLCV 데이터의 종가 기준 보유 성과입니다. 별도 지수 데이터가 아닙니다.</p><dl><div><dt>기준 수익률</dt><dd>{percent.format(result.benchmark_total_return)}</dd></div><div><dt>기준 최대 낙폭</dt><dd>{percent.format(result.benchmark_max_drawdown)}</dd></div><div><dt>초과 수익률</dt><dd>{percent.format(result.total_return - result.benchmark_total_return)}</dd></div></dl></article></section><section className="panel explanation-panel"><div className="panel-head"><div><span className="panel-kicker">AI INTERPRETATION</span><h2>결과 해석</h2></div>{!explanation && <button className="explain-button" onClick={loadExplanation} disabled={explaining}>{explaining ? "AI가 결과를 읽는 중..." : "AI 해석 보기"}</button>}</div>{explanation ? <><p className="explanation-summary">{explanation.summary}</p><div className="explanation-columns"><div><h3>관찰</h3><ul>{explanation.observations.map((item) => <li key={item}>{item}</li>)}</ul></div><div><h3>강점</h3><ul>{explanation.strengths.map((item) => <li key={item}>{item}</li>)}</ul></div><div><h3>위험</h3><ul>{explanation.risks.map((item) => <li key={item}>{item}</li>)}</ul></div></div><p className="disclaimer">{explanation.disclaimer}</p></> : <p className="explanation-placeholder">AI는 저장된 성과 지표만 설명합니다. 계산값과 거래 규칙은 변경하지 않습니다.</p>}{explanationError && <p className="error">{explanationError}</p>}</section><section className="panel data-version-panel"><span className="panel-kicker">REPRODUCIBILITY</span><h2>실행 데이터 버전</h2><p>{result.data_start_date} ~ {result.data_end_date} · {result.data_points}개 캔들</p><code>{result.data_version}</code></section><section className="panel trades-panel"><div className="panel-head"><div><span className="panel-kicker">TRADE LEDGER</span><h2>거래 내역</h2></div><span>{result.trades.length}개의 완료 거래</span></div><div className="table-wrap"><table><thead><tr><th>진입</th><th>청산</th><th>수량</th><th>보유</th><th>손익</th><th>수익률</th></tr></thead><tbody>{result.trades.map((trade) => <tr key={`${trade.entry_date}-${trade.exit_date}`}><td>{trade.entry_date}<small>{formatWon(trade.entry_price)}</small></td><td>{trade.exit_date}<small>{formatWon(trade.exit_price)}</small></td><td>{won.format(trade.quantity)}</td><td>{trade.holding_days}일</td><td className={trade.pnl >= 0 ? "positive" : "negative"}>{formatWon(trade.pnl)}</td><td className={trade.return_rate >= 0 ? "positive" : "negative"}>{percent.format(trade.return_rate)}</td></tr>)}</tbody></table></div></section></main>;
}

function LibraryView({ close }: { close: () => void }) {
  const [strategies, setStrategies] = useState<StrategyLibraryItem[]>([]); const [versions, setVersions] = useState<StrategyVersion[]>([]); const [selectedId, setSelectedId] = useState(""); const [loading, setLoading] = useState(true); const [error, setError] = useState("");
  useEffect(() => { getStrategyLibrary().then(setStrategies).catch((caught) => setError(caught instanceof Error ? caught.message : "전략 목록을 불러오지 못했습니다.")).finally(() => setLoading(false)); }, []);
  async function selectStrategy(strategyId: string) { setSelectedId(strategyId); setVersions([]); setError(""); try { setVersions(await getStrategyVersions(strategyId)); } catch (caught) { setError(caught instanceof Error ? caught.message : "버전 이력을 불러오지 못했습니다."); } }
  return <main className="library-shell"><div className="top-actions"><button className="text-button" onClick={close}>새 전략 만들기</button></div><section className="library-hero"><p className="eyebrow">ALPHALENS / STRATEGY LIBRARY</p><h1>확정한 규칙을<br />다시 꺼내봅니다.</h1><p>확정된 전략과 버전 이력만 보관합니다. 실행 결과와 원본 데이터는 별도 백테스트 기록으로 관리됩니다.</p></section>{loading ? <p>전략 보관함을 불러오는 중입니다.</p> : <section className="library-grid"><div className="library-list">{strategies.length === 0 ? <article className="empty-library">아직 확정된 전략이 없습니다.</article> : strategies.map((item) => <button key={item.strategy_id} className={`library-card ${selectedId === item.strategy_id ? "selected" : ""}`} onClick={() => selectStrategy(item.strategy_id)}><span>VERSION {item.latest_version}</span><strong>{item.strategy.strategy_name}</strong><small>{item.strategy.universe.symbols.join(", ")} · {item.strategy.market}</small><small>{new Date(item.confirmed_at).toLocaleDateString("ko-KR")} 확정</small></button>)}</div><aside className="version-panel">{selectedId ? <><span className="panel-kicker">VERSION HISTORY</span><h2>전략 이력</h2>{versions.map((version) => <article key={version.draft_id} className="version-card"><span>V{version.version} · {new Date(version.confirmed_at).toLocaleString("ko-KR")}</span><strong>{version.strategy.strategy_name}</strong><p>{version.strategy.entry_rules.conditions.map(conditionLabel).join(" / ")}</p><p>{version.strategy.exit_rules.conditions.map(conditionLabel).join(" / ")}</p></article>)}</> : <p className="empty-version">왼쪽에서 전략을 선택하면 확정 버전 이력을 표시합니다.</p>}</aside></section>}{error && <p className="error">{error}</p>}</main>;
}
