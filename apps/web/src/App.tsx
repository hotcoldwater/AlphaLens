...
import { confirmDraft, getBacktest, parseStrategy, runDraftBacktest } from "./api";
import type { BacktestResult, Condition, EquityPoint, OHLCVBar, StrategyDraft } from "./types";
...
function DraftView({ draft, onBack, onRun }: { draft: StrategyDraft; onBack: () => void; onRun: (data: OHLCVBar[]) => Promise<void> }) {
  ...
  return <main className="workflow-shell"><button className="text-button" onClick={onBack}>다시 입력하기</button><section className="workflow-hero"><p className="eyebrow">02 / REVIEW & RUN</p><h1>규칙을 확인하고,<br />실행합니다.</h1><p>AI가 만든 초안은 백테스트 전에 반드시 확인합니다. 계산은 확정된 구조화 규칙만 사용합니다.</p></section><section className="draft-grid"><article className="paper-card strategy-card"><span className="panel-kicker">STRATEGY DRAFT</span><h2>{draft.strategy.strategy_name}</h2><p className="strategy-meta">{draft.strategy.market} · {draft.strategy.universe.symbols.join(", ")} · {draft.strategy.period.start_date} ~ {draft.strategy.period.end_date}</p><div className="rule-block"><span>매수 조건</span>{draft.strategy.entry_rules.conditions.map((condition, index) => <strong key={index}>{conditionLabel(condition)}</strong>)}</div><div className="rule-block exit"><span>매도 조건</span>{draft.strategy.exit_rules.conditions.map((condition, index) => <strong key={index}>{conditionLabel(condition)}</strong>)}</div><p className="capital-line">초기 자본 <strong>{formatWon(draft.strategy.capital.initial_cash)}</strong></p></article><aside className="notes-card"><span className="panel-kicker">CHECK BEFORE RUN</span><h2>확인 사항</h2>{draft.assumptions.length > 0 && <><h3>적용한 가정</h3><ul>{draft.assumptions.map((item) => <li key={item}>{item}</li>)}</ul></>}{draft.warnings.length > 0 && <><h3>주의</h3><ul className="warnings">{draft.warnings.map((item) => <li key={item}>{item}</li>)}</ul></>}{draft.missing_fields.length > 0 && <p className="missing">추가로 정할 수 있는 항목: {draft.missing_fields.join(", ")}</p>}</aside></section><section className="data-card"><div><span className="panel-kicker">MARKET DATA</span><h2>검증용 가격 데이터</h2><p><strong>{source}</strong><br />CSV는 `date,open,high,low,close,volume` 헤더가 필요합니다. SMA 60 전략에는 최소 61개 이상의 캔들이 필요합니다.</p></div><label className="file-button">CSV 선택<input type="file" accept=".csv,text/csv" onChange={selectCsv} /></label></section>{error && <p className="error workflow-error">{error}</p>}<button className="run-button" onClick={submit} disabled={running}>{running ? "전략 확정 및 실행 중..." : `${data.length}개 캔들로 전략 확정 및 실행`}</button></main>;
}
...
export default function App() {
  ...
  if (draft) return <DraftView draft={draft} onBack={reset} onRun={execute} />;
