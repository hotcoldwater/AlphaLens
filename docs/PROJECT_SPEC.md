# AlphaLens 개발 명세서 & 로드맵

> 프로젝트 소개와 데모 링크는 저장소 루트의 [README.md](../README.md)를 참고하세요. 이 문서는 스키마·API·Phase별 구현 이력 등 세부 개발 기록입니다.
...
- 화면에서 공급원·종목을 직접 고르고 "시장 데이터 불러오기"를 눌러야 했던 수동 단계를 제거. 이제 "전략 확정 및 실행" 한 번으로 전략에 선언된 종목(및 교차자산 신호 종목)의 데이터를 공급원에서 자동 조회한 뒤 곧바로 백테스트를 실행. 공급원은 시장·심볼 형태로 자동 결정(`^`로 시작하는 지수는 Yahoo Finance, 그 외 KRX는 pykrx). 종목 검색 UI와 CSV 업로드는 제거했으며, 관련 백엔드 엔드포인트(`/market-data/symbols/search`)는 남아있지만 웹 클라이언트에서는 더 이상 사용하지 않음
- `pykrx`가 액면분할 등 기업행동 직후 거래정지일에 시가·고가·저가를 0으로, 종가만 이전값으로 채워 반환하는 경우가 있어(예: 삼성전자 2018년 액면분할 전후) OHLC 양수 검증에서 전체 조회가 실패하던 문제 수정 — 이런 행은 실제 거래일이 아니므로 조회 단계에서 걸러내고 나머지 정상 거래일로 진행 **완료**
...
### Phase 13. 매크로·해외지수 신호 확장

상태: **완료**

목표: 일반 투자자가 직관적으로 궁금해하는 "환율이 오르면", "미국 증시가 오르면" 같은 매크로·해외지수 조건을 단일 종목 전략의 신호로 쓸 수 있게 한다.

완료 범위:

- Phase 4에서 만든 교차자산 신호(`IndicatorReference.symbol`) 메커니즘을 그대로 재사용 — 새 스키마·엔진 변경 없이 원/달러 환율(`KRW=X`), 미국 10년물 금리(`^TNX`), 국제유가(`CL=F`), 금(`GC=F`), VIX(`^VIX`), 나스닥(`^IXIC`), S&P500(`^GSPC`), 필라델피아 반도체지수(`^SOX`) 등을 신호 종목으로 지원
- OpenAI 프롬프트에 한국어 표현("환율", "미국 금리", "유가", "변동성지수" 등)을 위 Yahoo Finance 티커로 매핑하는 지침 추가
- 프론트엔드 공급원 자동 라우팅을 "`^`로 시작하면 YFINANCE"에서 "6자리 숫자가 아니면 YFINANCE"로 일반화 — `KRW=X`, `CL=F`처럼 `^`로 시작하지 않는 티커도 정확히 Yahoo Finance로 라우팅
- 교차자산 신호 종목에 `=` 같은 특수문자가 섞여도 스키마·엔진·데이터 정렬이 정상 동작하는지 회귀 테스트로 검증
...
목표: 대중적으로 가장 관심이 높은 "외국인이 사면 따라 산다" 류의 전략을 지원한다.

⚠️ 착수 전 확인 필요: `pykrx`의 투자자별 매매동향 API(`get_market_trading_value_by_date` 등)가 조사 시점(2026-07-18) 기준 빈 데이터만 반환하는 것을 확인함 — OHLCV 조회는 정상 동작하는 것으로 보아 KRX 소스 변경으로 해당 기능만 깨졌을 가능성이 있음. 구현 착수 전 pykrx 최신 버전에서 복구 여부를 재확인하고, 안 되면 KRX 정보데이터시스템 공식 다운로드나 유료 공급원 등 대안을 검토해야 한다.

완료 조건:

- 외국인·기관·개인 순매수 금액/수량을 종목별 시계열로 조회하는 데이터 클라이언트
- `IndicatorType`에 `FOREIGN_NET_BUY`, `INSTITUTION_NET_BUY`, `INDIVIDUAL_NET_BUY` 등 추가, 연속 순매수일수 조건 지원
- OHLCV와 별도 도메인의 데이터를 정렬해 백테스트 엔진에 함께 공급하는 구조

### Phase 16. 재무제표·가치투자 지표

상태: **예정**

목표: PER, PBR, 배당수익률, ROE 등 재무 지표 기반 전략을 지원한다.

완료 조건:

- PER/PBR/배당수익률은 `pykrx get_market_fundamental_by_date`로 우선 지원
- ROE, 영업이익률 등 재무제표 세부 항목은 DART OpenAPI(무료, API 키 필요) 연동으로 확장

### Phase 17. 배당·공시 이벤트 전략

상태: **예정**

목표: 배당락일, 유상증자, 자사주매입, 액면분할, 신규상장, 지수 편입 등 이벤트 기반 조건을 지원한다.

완료 조건:

- DART OpenAPI 연동으로 공시 이벤트 조회
- 이벤트 발생일 기준 진입·청산 조건 스키마 확장

### Phase 18. 심리·뉴스·검색량 지표 (보류)

상태: **보류**

목표: 네이버 검색량, 뉴스량, 공포·탐욕지수 등 대체 데이터 기반 전략.

보류 사유: 구조화된 무료 API가 거의 없어 데이터 확보 난도가 높음. Phase 13~17 완료 후 재검토.
