...
단일 종목 전략의 매수·매도 조건은 기본적으로 거래 종목 자신의 데이터로 평가되지만, 조건의 지표에 `symbol`을 지정하면 KOSPI 등 다른 종목·지수를 신호로 사용할 수 있습니다. 신호 종목의 가격 데이터는 실행 전에 별도로 제공해야 하며, 조건이 신호 종목을 지정했는데 실행 요청에 해당 데이터가 없으면 거부됩니다.
