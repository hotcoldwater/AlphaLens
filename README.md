...
- 단일 종목 규칙과 2자산 국면 전환, 목표 비중, 월간 리밸런싱 Schema
...
- FMP 미국 주식 일봉 OHLCV 조회 API와 CSV 업로드 실행 API
...
- 자산별 목표 비중과 리밸런싱 주기 편집
- 수정 즉시 Schema 검증, 자연어 요약, 영향 경고 갱신

### Phase 10. 다중 자산 포트폴리오 고도화

상태: **예정**

목표: 2자산 전환을 일반적인 다중 자산 포트폴리오 규칙으로 확장한다.

완료 조건:

- 조건별 목표 포트폴리오와 현금 비중
- 리밸런싱 비용, 최소 주문 단위, 비중 허용 오차
- 자산별 데이터 누락 및 상장폐지 데이터에 대한 명시적 실행 거절
- 다중 자산 거래 원장과 자산별 성과 기여도
...
- FMP 수집 캐시, 수집 이력, 종목 메타데이터 저장
- KRX 승인 후 KOSPI/KOSDAQ 일봉 데이터 공급원 연결
- 공급원, 조정 방식, 해시, 수집 시각을 데이터 버전에 고정
- 티커 검색과 시장별 심볼 검증
...
목표: 다수 사용자가 사용할 수 있는 운영 안정성과 전략 소유권을 추가한다.

완료 조건:

- 중복 실행 방지, 실행 상태 관리, API 오류 응답 표준화
- 배포 환경 통합 테스트와 관측성
